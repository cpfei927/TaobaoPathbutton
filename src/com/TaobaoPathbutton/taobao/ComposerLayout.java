...
public class ComposerLayout extends RelativeLayout {
	...
	public void init(int[] imgResId, int showhideButtonId, int crossId, byte pCode, int radius,
		...
		if (pCode == CENTERBOTTOM || pCode == CENTERTOP) {
			...
			if (thislps.width != -1 && thislps.width != -2
					&& thislps.width < radius + mBottom.getWidth() + radius * 0.1) { // -1係FILL_PARENT，-2係WRAP_CONTENT
